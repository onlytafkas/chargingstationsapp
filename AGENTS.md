...
6. **VALIDATE**: Run `npm run test` (all pass), `npm run test:integration` (all pass), then `npm run test:coverage` (all business logic files ≥ 80% Stmts/Branch/Funcs/Lines). Run `npm run test:e2e` for changes that affect real browser flows. If coverage drops, add tests before finishing — do not skip this step.
